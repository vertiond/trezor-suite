...
import ElectrumModule from './index';
...
const desktopApiMock = () => {
    ...
    const call = (channel: string) => callbacks[channel] || (() => {});

    const electrumConnect = (server: string) => {
        socket = createSocket(server, { torAddress: TOR_ADDRESS });
        return socket
            .connect({
                onConnect: call('electrum/connected'),
                onClose: call('electrum/closed'),
                onReceive: call('electrum/received'),
                onEnd: call('electrum/ended'),
                onError: call('electrum/error'),
            })
            .then(() => true)
            .catch(() => false);
    };
    ...
    const electrumSend = (msg: string) => {
        socket?.send(msg);
    };

    const on = (channel: string, func: (...args: any[]) => any) => {
...
(async () => {
    ...
    const worker = ElectrumModule('test') || fail('No worker');
    if (!worker) return;
